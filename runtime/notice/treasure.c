...
unsigned int af_notice_treasure_mask(unsigned int id) {
    static const unsigned char masks[18] = {
        0x1E, 0x1A, 0x0C, 0x1A, 0x2A, 0x1C, 0x0A, 0x3A, 0x16,
        0x2A, 0x2A, 0x1A, 0x1E, 0x0C, 0x14, 0x1C, 0x14, 0x14
    };
    return id >= 0x1F0u && id <= 0x201u ? masks[id-0x1F0u] : 0u;
}

int af_notice_treasure_valid(const AfMailRecord *record) {
    unsigned int i, j, id, length;
    if (!record || ((__UINTPTR_TYPE__)record & 3u)
            || record->catalog != AF_MAIL_GLYPH_CATALOG_ID || record->kind
    ...
    for (i = 1; i <= 5u; ++i) {
        ...
        for (j = 0; j < field->length; ++j)
            if (field->text[j] == 0x7Fu || field->text[j] == 0x80u || field->text[j] == 0xCDu)
                return 0;
    }
    return 1;
}
...
int af_notice_treasure_pack(unsigned char *output, unsigned int capacity,
    ...
    return af_notice_treasure_valid(record) && af_notice_record_pack(output, capacity, record);
}
...
int af_notice_treasure_decode(AfNoticeWorkspace *work, const unsigned char *input,
                               unsigned int size) {
    if (!work || ((__UINTPTR_TYPE__)work & 15u)
            || overlap(work, sizeof(*work), input, size)) return 0;
    return af_notice_treasure_decode_parts(&work->mail, &work->letter, work->wire, input, size);
}

int af_notice_treasure_restore(AfNoticeText *output, const unsigned char *input,
    ...
    if (!output || !work || ((__UINTPTR_TYPE__)output & 3u)
            ...
            || !af_notice_treasure_decode(work, input, size)) return 0;

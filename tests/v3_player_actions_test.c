...
static int check_frame(void *fc, float frame) {
    assert(fc == (u8 *)actor + 0x174 && frame == 7.5f);
...
static void reset(void) {
    ...
    REAL(actor, 0x178) = 9; REAL(actor, 0x180) = 0.5f;
}
int main(void) {
    reset(); trigger_a = 1;
    ...
    for (int action = 7; action <= 11; ++action) {
        ...
        WORD(actor, 0xD04) = 0; REAL(actor, 0x180) = 0.499f;
    ...
    assert(frames[0] == 1 && frames[1] == 1 && frames[2] == 0.5f && frames[3] == -5);
    assert(setup_calls == 1 && eye == 5 && events[0] == 3 && events[1] == 4 && events[2] == 5);
    WORD(actor, 0xD58) = 0; af_v3_player_fan_setup(actor, game);
    assert(frames[0] == 1 && frames[1] == 12.5f && frames[2] == 0.5f && frames[3] == 0);

    reset(); WORD(actor, 0xD04) = 4; REAL(actor, 0x184) = 7; af_v3_player_fan_finish(actor, game);
    ...
    reset(); REAL(actor, 0x184) = 8; move_y = 0.3f; af_v3_player_fan_finish(actor, game);
    assert(walk_requests == 1 && !wait_requests && !settled);
    ...
    puts("shared player fan controls/setup/transitions pass");
    return 0;
}

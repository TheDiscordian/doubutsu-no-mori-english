"""Installed submenu leaf hook, retained native resources, and current composition."""
import json
...
from v3_furniture_icon import ABI, BLOB_SIZE, CODE, RAM, RELOC, START, SYMBOL, VROM, inspect

OUTPUT = ROOT / 'build/v3-furniture-icon-02'


@unittest.skipUnless((OUTPUT / 'build.json').exists(), 'Current local icon build required')
class FurnitureIconCartridgeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.report = json.loads((OUTPUT / 'build.json').read_text())
        cls.icon = cls.report['furniture_icon']
    ...
    def test_exact_window_and_complete_retained_owner(self):
        ...
        target = self.icon['code']['symbols'][SYMBOL]
        ...
        # In particular the mask/delay, early special cases, texture descriptor,
        # following relocated LUI/ADDIU pair, and renderer are all retained.
        self.assertEqual(old[0xDA4:0xDA8], bytes.fromhex('3219f000'))
        for changed in (bytes(expected), old[:-1]):
            with self.assertRaises(ValueError):
                inspect(code, changed, reloc)
        with self.assertRaises(ValueError):
            inspect(code, old, reloc[:-1])

    def test_resident_layout_query_and_existing_helpers(self):
        compiled = (OUTPUT / 'icon/code.bin').read_bytes()
        self.assertEqual(self.blob[CODE:CODE + len(compiled)], compiled)
        self.assertEqual(self.icon['code']['symbols']['af_v3_room_query'], 0x804680B8)
        self.assertLessEqual(CODE + len(compiled), BLOB_SIZE - 16)
        for part, offset in (('room', 0x8000), ('fields', 0xA400), ('menu', 0xA800)):
        ...
        self.assertEqual(sum(w >> 26 == 63 for w in words), 2)
        self.assertEqual(sum(w >> 26 == 55 for w in words), 2)

"""Shared daily hidden-content consumers and their current cartridge bindings."""
import json
import os
...
from aflib import CODE_VROM,by_vrom,sha256,apply_ups,u32
...
OUTPUT=ROOT/os.environ.get('V3_TREE_CONTENTS_BUILD','build/v3-shared-tree-contents-01')

class HostTests(unittest.TestCase):
    sanitized=shared.HostTests.sanitized
    def test_recording_counts_source_selection_and_bounds(self):self.sanitized('v3_tree_contents_test.c')

@unittest.skipUnless((OUTPUT/'build.json').is_file(),'Current tree-content cartridge required')
class CartridgeTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.e=cls.report['equipment_resources'];cls.r=cls.e['scenery'];cls.d=cls.r['hidden_contents']
        cls.blob=cls.files[BLOB].extract(cls.rom);cls.oldblob=cls.before[BLOB].extract(cls.base)

    def test_source_complete_native_schedulers_and_relocation(self):
        d=self.d;old=self.before[d['vrom']].extract(self.base);data=self.files[d['vrom']].extract(self.rom)
        ...
        evidence,bindings,records,_=scenery.contents_contract(source,old,oldrel,core)
        for k,value in evidence.items():self.assertEqual(json.loads(json.dumps(value)),d[k])
        changed=bytearray(old);changed[0xC30]^=1
        with self.assertRaises(ValueError):scenery.contents_contract(source,changed,oldrel,core)
        restored=bytearray(data)
        for p in d['patches']:
            self.assertEqual(u32(data,p['offset']),p['after']);self.assertEqual(u32(old,p['offset']),p['before'])
            struct.pack_into('>I',restored,p['offset'],p['before'])
        self.assertEqual(restored,old) # Native quantities, acre scheduling, and prior protections remain.
        self.assertEqual(len(d['removed_relocations']),4)
        kept=[v for v in records if v not in d['removed_relocations']]
        self.assertEqual(u32(rel,16),len(kept));self.assertEqual(rel[:16],oldrel[:16])
        self.assertEqual(struct.unpack_from('>'+str(len(kept))+'I',rel,20),tuple(kept))
        ...
        for at,target,_,link in bindings:
            if target<0x80AB0000:self.assertNotIn(at,{v&0xffffff for v in records})

    def test_shared_rebinding_storage_and_no_allocation_growth(self):
        ...
        self.assertEqual(self.r['additional_fixed_resident_bytes'],old['scenery']['additional_fixed_resident_bytes'])
        ...
        for row in self.r['owners']:
            restored=bytearray(self.files[row['vrom']].extract(self.rom));before=self.before[row['vrom']].extract(self.base)
            for p in row['patches']:
                self.assertEqual(u32(restored,p['offset']),p['after']);self.assertEqual(u32(before,p['offset']),p['before'])
                struct.pack_into('>I',restored,p['offset'],p['before'])
            self.assertEqual(restored,before)
            self.assertEqual(self.files[row['reloc']].extract(self.rom),self.before[row['reloc']].extract(self.base))

    def test_cartridge_composition_and_retained_saved_state(self):
        ...
        for h in self.d['core_hooks']:
            at=h['offset'];self.assertEqual(core[at:at+8].hex(),h['after']);self.assertEqual(before[at:at+8].hex(),h['before'])

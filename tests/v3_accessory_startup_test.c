...
static int transfers, failure, executions, writes, invalidations;
...
void af_v3_invalidate(void *p, u32 size) { (void)p; (void)size; ++invalidations; }
...
static void reset(void) {
    ...
    transfers = failure = executions = writes = invalidations = 0;
}
int main(void) {
    ...
    assert(writes == 2 && invalidations == 2 && !memcmp(art, af_v3_accessory_memory, sizeof(art)));

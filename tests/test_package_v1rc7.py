"""RC7 patch contents, committed stages, and correctly scoped human acceptance."""
import io
import json
...
from package_v1rc7 import FINAL_SHA, PREVIOUS_SHA, STAGES, prepare, verify_receipts


@unittest.skipUnless((ROOT/'build/gamestate-menu-text-01/fixes.json').exists(), 'Committed gamestate build required')
class RC7PackageTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.raws = [(ROOT/'build'/folder/'fixes.json').read_bytes()
                    for folder in ('title-warning-text-01', 'gamestate-menu-text-01')]

    def test_exact_members_hashes_and_scoped_acceptance(self):
        archive, manifest, image = prepare(ROOT/'build/gamestate-menu-text-01', ROOT/'build/title-warning-text-01',
        ...
        with zipfile.ZipFile(io.BytesIO(archive)) as bundle:
            ...
            self.assertNotIn(b'detection,+and', bundle.read('README.md'))
        for key in ('public_release', 'original_hardware_verified', 'ordinary_menu_appearance_verified',
                    ...
                    'human_playthrough_complete'):
            self.assertFalse(manifest[key])
        human = manifest['prior_human_acceptance']
        self.assertTrue(human['ordinary_save_restart_reload_confirmed'])
        self.assertEqual(human['confirmed_reported_findings'], [f'V1-{i:02}' for i in range(1, 24)])
        self.assertTrue((ROOT/human['evidence']).is_file())
        self.assertIn('before RC7', human['scope'])
        self.assertEqual(manifest['previous_candidate_sha256'], PREVIOUS_SHA)
        self.assertEqual(manifest['new_findings'], ['V1-26', 'V1-27'])
        self.assertEqual(manifest['save_compatibility']['previous_candidate'], 'V1RC6')
    ...
    def test_rejects_incomplete_dirty_disconnected_or_unbound_receipts(self):
        ...
        for index in range(2):
            for key, value in (('worktree_modified', True), ('source_builder_sha256', '0'*64),
                               ('saved_format_changed', True), ('save_readers_writers_changed', True),
                               ('menu_actions_changed', True), ('allocation_changed', True),
                               ('relocation_changed', True), ('required_ram_bytes', 0x400000),
                               ('baseline_sha256', '0'*64), ('source_revision', 'invalid')):
                rows = self.raws.copy()
    ...
    def test_allows_committed_replay_metadata_without_weakening_source_binding(self):
        ...
        with self.assertRaises(ValueError):
            prepare(ROOT/'build/gamestate-menu-text-01', ROOT/'build/title-warning-text-01', self.native, 'invalid')

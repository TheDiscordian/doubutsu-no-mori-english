# Accumulated completion-queue record

This is a preserved work record, not the active queue. Its build-specific
statuses and next steps are historical. Use [remaining work](WORK_QUEUE.md)
and [current progress](PROGRESS.md) instead.
...
their checked generation path without changing live scheduling or saved fields.
Do not credit the uninstalled candidate or overwrite structural zero-filled slots.
Then perform the bounded combined v0 checks and hand over the playtest patch.
...
builds, patch reconstruction, and the silent native check pass. Existing saved
catalogues and native font pixels stay unchanged. Continue residual general and
letter text; do not repeat the completed accent harness. Ordinary progression,
menus, and normal save/restart remain in the combined v0 smoke.
...
All four focused checks pass. Continue required sun/skull apology input, accents,
and residual general/letter text before the bounded combined v0 smoke.
...
checkpoint build is `build/borrowed-catchphrases-pilot`. All five focused checks and one
...
combined accounting. The completed `build/smoke-gyroid-items-01` batch passes
344 calls and 336 assertions, with restored checkpoint, blank isolated saves,
and graceful shutdown. Continue remaining names, wider callers, and other text.
...
The [Katrina phrase group](../specs/FORTUNE_STRINGS.md) installs all 128 complete
English fragments using the original random selections and existing sixteen-byte
item display. Its scoped native frame and string-data relocation add 108 ordinary
string candidates without widening other callers. All 810 host tests pass.
The silent native batch passes every phrase load/preparation, 32 complete readings,
...
Fifteen [native engine-test/sample/label drafts](../specs/ENGINE_DIAGNOSTICS.md)
retain every command, page, number, and menu target. All fifteen complete native
loads and 47 memory assertions pass with a restored checkpoint and blank saves.
Five larger Japanese test scripts remain: `0004/0005/000F/0010/0011`. Preserve
their deliberate glyph/sound samples while translating instructions, and use
approved page splits if complete English exceeds the buffer.
Next work covers other general strings and their callers, apology input,
and those five scripts. Apology `048E/0491`
...
intent. Rover's cancellation-enable/disable pair and final link, three furniture explanations,
...
native-specific drafts, preserving storage choices, prize ranks, offering
branches, and all four complete shop-twin echoes. Five focused tests, 96
reference tests, and eleven complete cartridge messages with six menu labels
...
Complete `14FD` uses an exact [source-glyph encoding approval](../specs/REFERENCE_GLYPH_ENCODING.md)
for the existing plus glyph, without a font change or rewritten controller tip.
All 109 reference tests pass. Its complete cartridge load passes in the
special-actor batch; ordinary conversation/rendering remains playthrough work.

Eleven [complete special-actor references](../specs/SPECIAL_ACTOR_SEQUENCES.md)
add Gracie, Redd, Jingle, Gulliver, Rover, and sound settings. Fourteen parts
retain full English wording and presentation; three individually checked
reserves supply long-message continuations. All non-expression actor requests
retain exact native order/multiplicity, including every earlier approved group.
Five focused tests, 110 reference tests, nine long-advice tests, and the silent
eighteen-message/eight-label batch pass. Normal actor progression, expressions,
gifts/services, sound-setting UI, final layout, and hardware remain unverified.

Five [contextual-expression references](../specs/CONTEXTUAL_ACTOR_SEQUENCES.md)
retain complete Booker, Gulliver, and Rover conversations using individually
bound same-character native expressions. All non-expression actions, name-entry
request/control, fields, and endings remain unchanged. Five focused tests, 110
reference tests, the earlier five special-actor tests, and five complete native
loads with both ending phases pass. Normal name entry, expressions, collection,
actor traversal, and presentation remain gameplay requirements.

The [complete train phone calls](../specs/TRAIN_PHONE_SEQUENCES.md) preserve
both long English scripts, protected openings, voice controls, and cancellation
behaviour through checked page-boundary splits. Five focused, ten pacing/advice,
110 reference, and ten special-actor tests pass. The silent batch verifies
42 native calls and 104 assertions, including actual message changes and normal
page setup for both cancellation states. Normal train progression and real input
timing remain required. No production runtime, font, or save change is needed.

Nine [native menu/dialogue follow-ups](../specs/NATIVE_MENU_FOLLOWUPS.md) retain
music-player actions, sound retry, fortune offer, NES launch, seat refusal,
snowfall, and the complete sleeping apology. Five focused, 110 reference, and
seven native-menu tests pass. The silent native batch passes twenty complete
messages, 21 labels, and 83 memory assertions with restored checkpoint and blank
...
unchanged native actor/action intervals; two `5808` endings retain the native
timer. Eight focused, 110 reference, ten earlier special-actor, and five train-phone
tests pass. The native batch passes nine full parts, 35 silent cue dispatches,
both timed endings, and 143 assertions with restored checkpoint and blank saves.
...
Current main-bank coverage is 10,825 candidates; 927 records remain without
candidates, including five containing Japanese static text. The reference
rejection queue contains 890 unconfirmed identities, fourteen control-signature

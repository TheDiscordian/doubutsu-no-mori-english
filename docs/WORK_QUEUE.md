...
that fit the current rules. Its report in `build/native-mood-identity-review/`
...
`build/native-mood-expression-review/queue.jsonl`, generated by
...
nineteen per-message mappings, including fourteen quiz menus and the reviewed
...
replies add nine missing messages. All 595 tests pass; the native batch checks
42 selected labels and insertions and 38 actual conditional branch cases,
with 302 calls, 484 assertions, restored checkpoint, and blank isolated saves.
Basic generation withholds sixteen mappings whose complete labels are missing;
do not bypass those dependencies. Broader contextual labels remain individual
review tasks, not a global label replacement.
Nine reviewed but withheld menu references and separate `1C6F` need native
question/label, meal-joke, map, birthday, old-calendar, or caller-date work,
...
sixteen-byte resource: 716 new slots. Current ordinary storage has 443 slots from
160 references; the wide resource has 1,365 slots from 461 references. The 126
...
resource reconstruction, UPS round trip, and all 671 full-suite tests pass,
including eleven focused item checks. The combined native loader batch passes
757 calls, 749 memory assertions, complete names, guards, and restored state.
Remaining name work covers ambiguous groups in the first 300, later furniture
groups, ordinary banks, and each unexpanded destination. Name identity is not
...
tests, the full 671-test suite, all actual installed edits, UPS application,
...
Nineteen [complete mood-preserving references](../specs/NATIVE_MOOD_REFERENCES.md)
retain both original mood/timer orders at individually matched English pages,
with all reference words, layout, and pauses unchanged. Eleven focused tests,
the 671-test suite, all installed payloads, UPS application, nineteen native
loads, and 38 native order dispatches pass. Normal mood/timer progression and
rendered conversations remain gameplay checks. Mid-page pairs in
`203A/262B/2637/264B/266B` need separate phrase-anchor review; changed-topic
`2773` is not approved. No general mood, actor, or random-branch permission is
granted by these page-start approvals.
...
Current main-bank coverage is 10,717 candidates; 1,035 records remain without
candidates, including 113 containing Japanese static text. The reference
rejection queue contains 929 unconfirmed identities, 81 control-signature
...
| R02 | Choice strings beyond ten bytes | active | All 460 choices have candidates; nine original labels retain native meanings, 164 dialogue approvals preserve native menu order, and nineteen contextual mappings retain answer indices/actions; 42 selected-label/insertion cases and 38 actual branch cases pass; twenty-byte capacity, thirteen long DMA loads, four rows, insertion, and cancellation pass targeted MIPS tests; further contextual labels, full review, and actor-specific runtime paths remain |
...
| R04 | NPC and item names | active | 178 six-byte villager names and all native loads pass; eight-byte API covers all 216 villagers and 64 special-actor rows; main insertion, two nameplate consumers, eight rendered quads, and guards pass native tests; combined train-to-town regression passes; 160 item reference IDs occupy 443 ten-byte slots, and 461 references occupy 1,365 wider-resource slots; all 4,547 item-ID cases have earlier passing runs, with one initial stop retained; the new complete-name batch passes 757 native calls and restored-state checks, with one shorter-budget incomplete run retained; sixteen-byte main item fields, one item-ID wrapper, and a confirmed clothing alias pass native tests; other destinations and identities remain |
...
| R07 | Remaining message matching | active | Hash-bound identities, complete reference and native-original sequences, 41 unanimous complete-native-record aliases, and 74 separately guarded cross-bank letter references implemented; all 360 exact development-label slots accounted for, including fifteen guarded continuation allocations; 99 complete diagnostic labels covered; 113 Japanese-static-text gaps and broader matching remain |

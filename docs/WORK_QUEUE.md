...
that fit the current rules. Its report in `build/return-greetings-identity-review/`
still identifies 1,093 without visible same-ID English, ten requiring fields,
...
`build/return-greetings-expression-review/queue.jsonl`, generated by
...
Continue adjacent `00A3..00AF` month-return conversations and the earlier native
greeting/test range without treating any record as unreachable from its ID.

Current main-bank coverage is 10,447 candidates; 1,305 records remain without
candidates, including 383 containing Japanese static text. The reference
rejection queue contains 1,169 unconfirmed identities, 111 control-signature
...
| R07 | Remaining message matching | active | Hash-bound identities, complete sequences, 41 unanimous complete-native-record aliases, and 74 separately guarded cross-bank letter references implemented; all 360 exact development labels covered without reusing sequence slots; 383 Japanese-static-text gaps and broader matching remain |

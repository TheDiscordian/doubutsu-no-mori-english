...
| R04 | NPC and item names | active | 178 six-byte villager names and all native loads pass; separate eight-byte API covers all 216 villagers and 64 special-actor rows with native DMA/guard tests; dialogue integration remains; 107 item reference IDs occupy 231 ten-byte slots; all 4,547 item-ID cases have passing runs, with one initial stop retained; sixteen-byte main item fields, one item-ID wrapper, and a confirmed clothing alias pass native tests; other destinations and identities remain |

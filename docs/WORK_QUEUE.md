...
that fit the current rules. Its report in `build/native-menus-identity-review/`
...
`build/native-menus-expression-review/queue.jsonl`, generated by
...
Next menu work covers contextual `00B7` (none/pear), `0010` (agreement/refusal),
and `00CD/00CE` (quiz truth versus shapes) labels. Do not globally replace the
Circle/X labels: 35 menus in 33 records share them, including shape games.
Fourteen reviewed but withheld menu references and separate `1C6F` need native
...
Current main-bank coverage is 10,403 candidates; 1,349 records remain without
candidates, including 427 containing Japanese static text. The reference
rejection queue contains 1,204 unconfirmed identities, 120 control-signature
...
| R02 | Choice strings beyond ten bytes | active | All 460 choices have candidates; nine original labels retain native meanings, and 159 dialogue approvals preserve native menu order; the latest 168-message/126-label native load batch passes; twenty-byte capacity, thirteen long DMA loads, four rows, insertion, and cancellation pass targeted MIPS tests; contextual labels, full review, and actor-specific runtime paths remain |
...
| R07 | Remaining message matching | active | Hash-bound identities, complete sequences, 41 unanimous complete-native-record aliases, and 74 separately guarded cross-bank letter references implemented; all 360 exact development labels covered without reusing sequence slots; 427 Japanese-static-text gaps and broader matching remain |

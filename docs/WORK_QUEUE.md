...
that fit the current rules. Its report in `build/native-misc-identity-review/`
identifies 866 without visible same-ID English, three requiring fields,
twelve with control differences, seven overflows, 36 native non-static records,
...
`build/native-misc-expression-review/queue.jsonl`, generated by
...
Continue the four complete native Controller Pak/travel explanations
`0848/0866/0870/087A`. The supplied GameCube references add second-Memory-Card
and destination-town-data rules that are not the N64 instructions. Preserve
the native advice, pauses, fields, and pages; `0848` also explains bulletin-board
messages. If complete English exceeds capacity, add a properly guarded split
at an existing page boundary instead of shortening the explanation. Do not
replace compatible glyph-blocked references such as `04D2/04FA` with native
drafts merely to avoid implementing punctuation.
...
Current main-bank coverage is 10,692 candidates; 1,060 records remain without
candidates, including 138 containing Japanese static text. The reference
rejection queue contains 935 unconfirmed identities, 100 control-signature
...
| R07 | Remaining message matching | active | Hash-bound identities, complete sequences, 41 unanimous complete-native-record aliases, and 74 separately guarded cross-bank letter references implemented; all 360 exact development labels and 99 complete diagnostic labels covered without reusing sequence slots; 138 Japanese-static-text gaps and broader matching remain |

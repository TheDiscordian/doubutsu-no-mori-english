...
that fit the current rules. Its report in `build/resident-topics-identity-review/`
still identifies 1,461 without visible same-ID English, 27 requiring fields,
17 with control differences, seven overflows, 36 native non-static records,
24 glyph failures, and one encoding/hash uncertainty. These are review routes,
...
`build/resident-topics-expression-review/queue.jsonl`, generated by
...
Audit reserved-source imports next: the remaining 61 automatic alias conflicts
contain only three train-demo reserved labels and 58 exact `よび` labels. Do not
choose an English donor just because these short placeholder sources match.
Existing mechanically matched donors include `07DA`, `2AFF..2B04`, and `2B42`;
the last contains a Game Boy Advance instruction absent from the native label.
Review native callers and complete sources, correct inappropriate imports, and
classify reserved labels explicitly without claiming that they are unreachable.
The current classifier recognises exact `ダミー` labels, but not `よび`.

Current main-bank coverage is 9,881 candidates; 1,871 records remain without
candidates, including 940 containing Japanese static text. The reference
rejection queue contains 1,572 unconfirmed identities, 273 control-signature
differences, 25 missing-field records, and two expansion overflows. These are
...
The 25 missing-field rejections include remaining catchphrases (`1C`), player
...
| R07 | Remaining message matching | active | Hash-bound identities, complete sequences, and 41 unanimous complete-native-record aliases implemented; all functional Nook introductory-job records have candidates; 61 reserved-label alias conflicts, inappropriate placeholder imports, and broader matching remain |

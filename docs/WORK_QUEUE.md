...
that fit the current rules. Its report in `build/startup-clocks-identity-review/`
...
`build/startup-clocks-expression-review/queue.jsonl`, generated by
...
Next native-topic candidates include `087B/1506` (villagers moving through the
player's travel, not GameCube move-out persuasion), `2A43` (winter flowers, not
an unavailable hobby field), and `2BC3/2BC5/2BC7/2BC9` (native first introductions,
not GameCube previous-town field `36`). The three daytime introduction sources
are identical; the late-night source is different. Translate complete native
meaning and retain each original command sequence. Review connected records as
a batch; do not insert GameCube dialogue into native command-only placeholders.

Current main-bank coverage is 9,859 candidates; 1,893 records remain without
candidates, including 962 containing Japanese static text. The reference

...
that fit the current rules. Its report in `build/startup-greetings-identity-review/`
still identifies 1,461 without visible same-ID English, 33 requiring fields,
...
`build/startup-greetings-expression-review/queue.jsonl`, generated by
...
Finish the six opening clock greetings `13F2/141A/1442/146A/1492/14BA`, the only
remaining Japanese static text in `13F2..14E1`. Their GameCube `28` field names
storage that is not prepared by the N64 message. Preserve the reference calendar,
AM/PM, manual lines, and pauses while removing only that unavailable device
clause under full source/output checks. Do not relax general field availability.
First ensure original drafts with module commands are withheld by basic
generation; the full builder already verifies the installed runtime and rejects
unsupported tokens. Broader native-topic translation remains available too.

Current main-bank coverage is 9,853 candidates; 1,899 records remain without
candidates, including 968 containing Japanese static text. The reference
rejection queue contains 1,578 unconfirmed identities, 273 control-signature
differences, 47 missing-field records, and two expansion overflows. These are
...
The 47 missing-field rejections include remaining catchphrases (`1C`), player
...
| R07 | Remaining message matching | active | Hash-bound identities, complete sequences, and 41 unanimous complete-native-record aliases implemented; all functional Nook introductory-job records have candidates; 68 alias conflicts and broader matching remain |

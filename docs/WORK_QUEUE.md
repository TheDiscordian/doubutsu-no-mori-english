...
category run pass; one fixture correction accounts for byte-sized native counts.
...
The lawn drag program and extended movement table grow the sequence by 176
bytes, without new instruments/samples. Checked audio-heap growth is 1 KiB;
conservative spare capacity is 896 bytes. Three focused host checks and a
58-assertion native run pass; one fixture retry accounts for the original
sequence's explicit self-modifying dispatch operands. Native synthesis,
listening, ordinary room interaction, and hardware remain unverified.
Retain passing unchanged scoring/menu/save evidence.

Full-index HRA surface
...
Floor audio uses checked complete program/instrument matching, not numerical
aliases. The full-index tables retain all 73 original selectors. Native walking
variants have stride nine, while donor variants have stride ten. Preserve the
native stride, complete original consumer behaviour, and separate raw movement
...
and saves remain intact. One sanitizer and four cartridge/composition checks
...
The private export's review list comes from its donor scan: 164 unavailable furniture
...
is added. The eight prepared models retain 29,664 artwork bytes, 555 vertices, and
346 triangles. Fifty-seven focused checks pass across the combined run and

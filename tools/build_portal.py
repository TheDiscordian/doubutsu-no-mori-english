"""Export a private static site and a genuine two-game reconstruction recipe."""
import argparse
import gzip
import json
...
def main():
    ...
    if sha256(target) != TARGET_SHA or struct.unpack_from('>2I', target, 0x10) != n64_checksum(target):
        raise ValueError('Expected the checksummed V2-07 map-suffix correction cartridge')

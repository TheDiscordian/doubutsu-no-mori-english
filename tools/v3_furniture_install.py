...
def refresh_runtime(output, lock=LOCK, *, equipment_art=None, player_motion=False, equipment_kinds=False,
    ...
    if equipment_report:
        ...
        if scenery_gameplay:
            ...
            adapter='scenery_felling_camera' if equipment_report['scenery'].get('felling_camera') else 'scenery_player_queries' if player_tree else 'scenery_interactions' if interaction else 'scenery_world_queries' if world else 'scenery_hidden_contents' if contents else 'scenery_daily_growth' if daily else 'scenery_tree_states'

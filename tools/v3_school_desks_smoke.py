"""One combined current desk-reader, full-model DMA, placement, and catalogue check."""
import json
...
import v3_school_desks_runtime as runtime
...
def exercise(debug, rom_path, record):
    ...
    if sha256(image) != report['output_sha256'] or report['runtime_abi'] != 84:
        raise ValueError('Desk probe requires its current ABI-84 cartridge')
    ...
    def check(label, address, expected):
        ...
        record(dict(school_desk_check=label, address=f'{address:08X}', bytes=len(expected),
                    assertion='passed' if actual == expected else 'failed',
                    expected_sha256=sha256(expected), observed_sha256=sha256(actual)))
        if actual != expected: raise ValueError('Native desk mismatch: ' + label)
    ...
    def call(address, args=(), expected=None, proof=None):
        ...
        if expected is not None and result['return_value'] != expected:
            raise ValueError(f'Native desk call {address:08X}: unexpected result')
    ...
    if allocation & 15 or not MODULE_RAM + 0x8000 <= allocation <= 0x80400000 - size:
        raise ValueError('Desk fixture allocation outside native heap')
    ...
    def load(vrom, reloc_vrom, ram, resident):
        ...
        if resident + len(reloc) >= scratch - owner - 16:
            raise ValueError('Native desk owner exceeds its fixture reservation')
    ...
    try:
        for row in report['school_desks']['imports']:
            ...
            for rotation in (range(4) if item == 0x3220 else (0,)):
        ...
        for n, row in enumerate(report['school_desks']['imports']):
    ...
    return dict(native_school_desk_readers=True, complete_native_model_dma=True,

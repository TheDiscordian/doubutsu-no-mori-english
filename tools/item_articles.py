...
# Version-one immutable profile. Change only with a fresh complete name/article
# source audit, not by trusting a replacement manifest's self-reported checksum.
...
def build(rom, names, report, rel, symbols, originals):
    ...
    if (report.get('source_sha256') != sha256(rom) or report.get('data_sha256') != sha256(names)
            or sha256(names) != NAMES_HASH or name_resource(rom, report['edits']) != names):
    ...
    output, rows, slot = bytearray(HEADER), [], 0
...
def verify(data):
    if len(data) != SIZE or data[:48] != HEADER or DATA_HASH is None or sha256(data) != DATA_HASH:
        raise ValueError('Changed or unapproved complete item article profile')


def verify_names(names, configured_vrom):
    if configured_vrom != 0x02A00000 or sha256(names) != NAMES_HASH:

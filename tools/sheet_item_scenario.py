#!/usr/bin/env python3
"""One native batch for newly installed, source-bound complete item names."""
...
from item_matches import load_matches, validate_candidate, SHEET_APPROVALS
...
def scenario(native, built, module, names):
    approvals = {r['id'] for r in json.loads(SHEET_APPROVALS.read_text())}
    selected = [r for r in names['edits'] if r.get('item_reference_match') in approvals]
    if len(approvals) != 327 or len(selected) != 834 or {r['item_reference_match'] for r in selected} != approvals:
        raise ValueError('Sheet-name batch requires all 327 identities and 834 complete fields')
    ...
    if len(short) != 112: raise ValueError('Sheet-name batch requires all 112 complete short fields')
...
def main():
    ...
    parser.add_argument('--build', type=Path, default=Path('build/sheet-items-pilot'))
    parser.add_argument('--names', type=Path, default=Path('build/sheet-items-resource/names.json'))
    parser.add_argument('--output', type=Path, default=Path('build/sheet-items-scenario.json'))

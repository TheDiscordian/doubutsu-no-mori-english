#!/usr/bin/env python3
"""Append complete English renewal delivery to its original general-heap actor."""
...
from renewal_actor import (RAM,PREFIX_BYTES,native_sources,source_hashes,imports_for,
...
def build(rom,module,creator_code,creator,out):
    ...
    def run(tool,*args):
        ...
        if result.returncode: raise ValueError(f'Renewal {tool} failed: '+result.stdout+result.stderr)
    ...
    run('gcc',*flags,'/source/overlays/mail_generation/renewal_actor.c','-o','renewal_actor.o')
    run('as','-EB','-mabi=32','-march=vr4300','-I/out','-o','native.o',
        '/source/overlays/mail_generation/renewal_actor.s')
    create_entry = RAM+PREFIX_BYTES+creator['symbols']['af_leaflet_create']
    run('ld','-EB','--emit-relocs','-T','/source/overlays/mail_generation/renewal_actor.ld','-Map=overlay.map',
        *(f'--defsym={name}=0x{value:08X}' for name,value in imports.items()),
        f'--defsym=af_leaflet_create=0x{create_entry:08X}',
        '-o','overlay.elf','native.o','renewal_actor.o')
    if run('nm','--undefined-only','overlay.elf').strip(): raise ValueError('Undefined renewal symbol')
    symbols = {}
    ...
    if (symbols['__renewal_start'] != RAM or symbols['__renewal_end'] != RAM+len(data)
            or symbols['__renewal_creator'] != RAM+PREFIX_BYTES):
        raise ValueError('Renewal linked bounds disagree with extracted code')
    ...
    report = {'version':1,'ram':RAM,'bytes':len(data),'overlay_sha256':sha256(data),
              'relocation_bytes':len(reloc),'relocation_sha256':sha256(reloc),
              ...
              'flags':flags,'toolchain_image':IMAGE,'stack_usage':(out/'renewal_actor.su').read_text(),
              'status':'Complete renewal publication code; installed native and gameplay validation remain separate'}
    if source_hashes() != sources: raise ValueError('Renewal sources changed while building')
...
def main():
    ...
    parser.add_argument('--creator',type=Path,default=Path('build/leaflet-letters-probe'))
    parser.add_argument('--output',type=Path,default=Path('build/renewal-actor'))

#!/usr/bin/env python3
"""Build complete-letter shop owners within their two original main-code slots."""
...
from shop_notice_letters import ROOT,START,sources,verify_code,validate
...
def build(native,module,out):
    ...
    def run(tool,*args):
        ...
        if result.returncode: raise ValueError('Shop notice '+tool+' failed: '+result.stdout+result.stderr)
        return result.stdout
    run('as','-EB','-mabi=32','-march=vr4300','-o','owners.o','/source/overlays/mail_generation/shop_notice_entry.s')
    run('ld','-EB','-Ttext',f'0x{START:08X}','-e','shop_spotlight_entry',
        f'--defsym=af_npc_mail_load=0x{loader:08X}','-o','owners.elf','owners.o')
    if run('nm','--undefined-only','owners.elf').strip(): raise ValueError('Unresolved shop notice helper')
    ...
    for line in run('nm','--defined-only','owners.elf').splitlines():
        fields = line.split()
        if len(fields)==3 and fields[2] in ('shop_spotlight_entry','shop_spotlight_end','shop_reopening_entry','shop_reopening_end'):
...
def main():
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'build/shop-notice-owners')

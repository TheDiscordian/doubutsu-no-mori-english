...
def scenario(rom, source, edits):
    ...
    for record in approved:
        ...
        command = bytes.fromhex(record["native_actor_request"]["native_command"])
        targets = [t for t in tokenize(entry, info) if t.kind == "cmd" and t.data == command]
        if len(targets) != 1:
            raise ValueError("Expected exactly one approved native request")
        token = targets[0]
        ...
        # Header + ten rows of ten u16 entries, with adjacent guards. The NPC0
        # row-four slot-five value changes; quest row nine must remain untouched.
        expected = bytearray(b"G"*(16+216+16))
        offset = 16+16+4*20+5*2

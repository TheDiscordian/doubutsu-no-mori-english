...
from v3_villager_defaults import BRIDGES as DEFAULT_BRIDGES, clothing
...
def metadata(native, donor, first, symbols):
    ...
    for index in (232, 235):
        ...
        record = (struct.pack('>HH4B', actor, cloth, looks[index], umbrella, growth[index], 1)
                  + name.ljust(8, b' ') + phrase.ljust(10, b' ') + key + struct.pack('>H', native_cloth))
        slot = actor - 0xE0DA
...
def install(native, code, module, blob, symbols, donor, first, donor_symbols):
    ...
    if len(blob) != BLOB_SIZE or any(blob[DATA:0x2F80]):
    ...
    return {'imports': rows, 'hooks': patches, 'metadata_ram': f'{0x80460000 + DATA:08X}',
            ...
            'remaining': ['Punchy cherry-shirt import and initial defaults', 'umbrella asset identity',
                          'house data', 'remaining ID-bounded readers',

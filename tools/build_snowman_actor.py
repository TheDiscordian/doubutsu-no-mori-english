#!/usr/bin/env python3
"""Append complete fixed Snowman letters with no new per-letter allocations."""
...
from snowman_actor import ROOT,RAM,native_sources,source_hashes,elf_inventory,patch_prefix,relocation_bytes,validate
from snowman_snapshots import snapshots


def build(native,module,catalog,items,out):
    original,native_reloc = native_sources(native);table,prepared = snapshots(native,catalog,items)
    ...
    def run(tool,*args):
        ...
        if result.returncode: raise ValueError(f'Snowman {tool} failed: '+result.stdout+result.stderr)
    ...
    run('gcc',*flags,'/source/overlays/mail_generation/snowman_creator.c','-o','creator.o')
    run('as','-EB','-mabi=32','-march=vr4300','-I/out','-o','native.o','/source/overlays/mail_generation/snowman_actor.s')
    run('ld','-EB','--emit-relocs','-T','/source/overlays/mail_generation/snowman_actor.ld','-Map=overlay.map',
        '-o','overlay.elf','native.o','creator.o')
    if run('nm','--undefined-only','overlay.elf').strip(): raise ValueError('Unresolved Snowman creator import')
    symbols = {}
    ...
    if symbols['__snowman_start']!=RAM or symbols['__snowman_end']!=RAM+len(data):
        raise ValueError('Snowman linked bounds disagree')
    ...
    report = {'version':1,'ram':RAM,'bytes':len(data),'overlay_sha256':sha256(data),
              ...
              'snapshots':prepared,'symbols':exports,'code_end':symbols['__snowman_code_end']-RAM,
    ...
    if sources!=source_hashes(): raise ValueError('Snowman sources changed during build')
    validate(native,bytes(data),reloc,report,module,catalog,items)
...
def main():
    ...
    parser.add_argument('--module',type=Path,default=ROOT/'build/runtime-module/module.json')
    ...
    parser.add_argument('--output',type=Path,default=ROOT/'build/snowman-actor')
    args = parser.parse_args()
    report = build(verified_rom(args.rom.read_bytes()),json.loads(args.module.read_text()),
                   args.catalog.read_bytes(),args.items.read_bytes(),args.output.resolve())

...
ACCENT_GLYPHS = MAIL_GLYPHS+(('ñ',0x87,True),('Ñ',0x12,True))
...
def glyph_profile(mail, accents):
    if accents and not mail:
        raise ValueError('Accent glyphs require the complete preceding mail font')
    return ACCENT_GLYPHS if accents else MAIL_GLYPHS if mail else GLYPHS


def source_atlas(rel, symbols, decoder, *, mail=False, accents=False):
    ...
    if any(table[code] != text for text, code, _ in glyph_profile(mail,accents)):
    ...
    if mail:
        ...
        if accents and pairs.get(0x87)!=0x12:
            raise ValueError('Changed supplied n-tilde capitalization')
    return untile_i4(raw, 192, 256)


def resource(atlas, *, mail=False, accents=False):
    ...
    result = [0]*(192*16)
    widths, rows = [], []
    glyphs = glyph_profile(mail,accents)
    ...
    validate_resource(data,mail=mail,accents=accents)
    return data, rows


def validate_resource(data, *, mail=False, accents=False):
    glyphs = glyph_profile(mail,accents)
    header = struct.pack('>8I',0x41464758,1,192,16,len(glyphs),64,1536,0)
...
def main():
    ...
    parser.add_argument('--accents',action='store_true',help='Include exact n-tilde/uppercase cells; requires --mail')
    args = parser.parse_args()
    symbols = (args.decomp/'config/GAFE01_00/foresta/symbols.txt').read_text()
    data, glyphs = resource(source_atlas(args.rel.read_bytes(), symbols, args.decomp/'tools/msg_tool.py',
        mail=args.mail,accents=args.accents),mail=args.mail,accents=args.accents)
    ...
    if args.accents:report['accent_glyphs']=True

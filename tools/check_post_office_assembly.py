#!/usr/bin/env python3
"""Independently assemble the postal creator and both native receipt gates."""
...
from post_office_letters import ROOT,START,patches


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--output',type=Path,default=Path('build/post-office-assembly'))
    ...
    source = ROOT/'overlays/mail_generation';digest = sha256((source/'post_office_entry.s').read_bytes())
    ...
    run('as','-EB','-mabi=32','-march=vr4300','-o','entry.o','/source/post_office_entry.s')
    run('ld','-EB','-Ttext',f'0x{START:08X}','-e','post_office_entry',
    ...
    for at,value in expected.items():
        if data[at-START:at-START+len(value)] != value:
            raise ValueError(f'Post-office assembler mismatch at {at:08X}')
    if sha256((source/'post_office_entry.s').read_bytes()) != digest:
        raise ValueError('Post-office assembly source changed during comparison')

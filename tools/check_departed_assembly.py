#!/usr/bin/env python3
"""Independently assemble the departed-villager creator and receipt gate."""
...
from departed_letters import ROOT,START,POST,END,patch


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--output',type=Path,default=Path('build/departed-assembly'))
    ...
    source = ROOT/'overlays/mail_generation';digest = sha256((source/'departed_entry.s').read_bytes())
    ...
    run('as','-EB','-mabi=32','-march=vr4300','-o','entry.o','/source/departed_entry.s')
    run('ld','-EB','-Ttext',f'0x{START:08X}','-e','departed_entry',
    ...
    for start,end in ((START,POST),(0x800B9E00,0x800B9E34)):
        if data[start-START:end-START] != expected[start-START:end-START]:
            raise ValueError(f'Departed entry assembler mismatch at {start:08X}')
    if sha256((source/'departed_entry.s').read_bytes()) != digest:
        raise ValueError('Departed entry source changed during assembly')
    report = {'assembly_sha256':digest,'patch_sha256':sha256(expected),'toolchain_image':IMAGE,
              'creator_bytes':POST-START,'publication_bytes':52,'loader_ram':f'{loader:08X}'}

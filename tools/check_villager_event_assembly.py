#!/usr/bin/env python3
"""Independently assemble shared event/Christmas entries and publication gates."""
...
from villager_event_letters import ROOT,START,patches


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--output',type=Path,default=Path('build/villager-event-assembly'))
    ...
    source = ROOT/'overlays/mail_generation';digest = sha256((source/'villager_event_entry.s').read_bytes())
    ...
    run('as','-EB','-mabi=32','-march=vr4300','-o','entry.o','/source/villager_event_entry.s')
    run('ld','-EB','-Ttext',f'0x{START:08X}','-e','villager_event_entry',
    ...
    for at,value in expected.items():
        if data[at-START:at-START+len(value)] != value:
            raise ValueError(f'Villager-event assembler mismatch at {at:08X}')
    if sha256((source/'villager_event_entry.s').read_bytes()) != digest:
        raise ValueError('Villager-event assembly source changed during comparison')

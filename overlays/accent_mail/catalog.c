...
int af_mail_catalog_header_valid(const unsigned int *header, unsigned int catalog) {
    static const unsigned int fingerprints[4][8] = {
    ...
    if (!header || !af_mail_catalog_vrom(catalog) || header[0] != 0x41464D4Cu
            ...
            || header[3] != (catalog == AF_ACCENT_CATALOG_ID ? 3u : catalog == AF_MAIL_GLYPH_CATALOG_ID ? 2u : 1u)
...
static int fields(const unsigned char *data, unsigned int size, unsigned int *mask, unsigned int catalog) {
    ...
    while (pos < size) {
        code = data[pos++];
        if (code == 0x80u) {
            if ((catalog != AF_MAIL_GLYPH_CATALOG_ID && catalog != AF_ACCENT_CATALOG_ID) || pos == size || !af_accent_glyph_width(data[pos++],catalog))
                return 0;
            continue;
        }
        if (code != 0x7Fu)

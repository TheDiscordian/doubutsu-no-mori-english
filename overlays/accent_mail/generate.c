...
#define af_mail_capture_reset af_accent_capture_reset
#define af_mail_capture_set af_accent_capture_set
#define af_mail_generate af_accent_mail_generate
#define af_mail_catalog_vrom af_accent_catalog_vrom
#define af_mail_catalog_header_valid af_accent_catalog_header_valid
#define af_mail_restore af_accent_mail_restore
...
int af_mail_capture_set(AfMailCapture *capture, unsigned int slot,
    ...
    if ((!text && length) || length > AF_MAIL_FIELD_BYTES || article > 4)
        goto invalid;
    for (i = 0; i < length; ++i)
        if (text[i] == 0x7Fu || (text[i] == 0x80u && !af_accent_item_literal(text,length)))
            goto invalid;
...
int af_mail_generate(unsigned char *mail, unsigned int mail_size, AfMailCapture *capture,
    ...
    /* Only selected accent-bearing fields need the new immutable interpretation. */
    for (i=0;i<AF_MAIL_FIELD_COUNT;++i) if (mask & (1u<<i)) {
        if (capture->fields[i].length>AF_MAIL_FIELD_BYTES) return 0;
        for (j=0;j<capture->fields[i].length;++j) if (capture->fields[i].text[j]==0x80u) {
            /* Native shop leaflets select catalogue two. These sixteen full
             * templates are identical in catalogues two/four/five; the builder
             * verifies all three parts before permitting this narrow upgrade. */
            if (selection->catalog!=AF_MAIL_GLYPH_CATALOG_ID && selection->catalog!=AF_ACCENT_CATALOG_ID
                    && !(selection->catalog==AF_MAIL_CATALOG_ID && !selection->kind
                         && selection->templates[0]>=2u && selection->templates[0]<=17u)) return 0;
            record->catalog=AF_ACCENT_CATALOG_ID;
        }
    }

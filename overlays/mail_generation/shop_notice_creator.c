...
int af_shop_notice_mail_create(AfNpcMailCreateWork *work,unsigned char *destination,
                               AfNpcMailSession **active,unsigned int *capital) {
    AfNpcMailSession *session;
    const unsigned char *request,*player;
    unsigned int i,number,item;
    ...
    if (session->remail || !request)
        return af_museum_mail_create(work,destination,active,capital);
    if ((__UINTPTR_TYPE__)request&1u) return 0;
    if (request[11] != 247u)
        return af_museum_mail_create(work,destination,active,capital);
    if (!player || ((__UINTPTR_TYPE__)player&1u) || session->condition || session->foreign
            || request[0] != 'A' || request[1] != 'F' || request[2] != 'S' || request[3] != 'N'
            || request[8] != 55u || request[9] > 1u || request[10]) return 0;
    number = ((unsigned int)request[4]<<8)|request[5];
    item = ((unsigned int)request[6]<<8)|request[7];
    if (number >= 0x12u && number <= 0x17u) {
        if (!item) return 0;
    } else if (number < 0x1Bu || number > 0x1Du || item) return 0;
    ...
    if (number >= 0x14u && number <= 0x17u
            && (!af_load_item_name(text,16u,item)
                || !af_mail_capture_set(&work->captured.capture,7u,text,16u,0u))) return 0;
    for (i = 0; i < 12u; ++i) work->stage[i] = ' ';
    for (i = 12u; i < 17u; ++i) work->stage[i] = 255u;
    if (request[9]) {
    ...
    work->stage[40] = 2u;work->stage[41] = 55u;
